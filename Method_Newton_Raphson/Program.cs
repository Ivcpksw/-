...
namespace Method_Newton_Raphson
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Method_Newton_Raphson(Point_1, Point_2, eps_1, eps_2, M);
        }

        public static double Max_X(double x, double Point_1, double t0)
        {
            return Point_1 + (x * t0);
        }

        public static double Max_Y(double y, double Point_2, double t0)
        {
            return Point_2 + (y * t0);
        }
        ...
        // Расскрываем скобки в ф-ии + значение t0
        ...
        public static void Method_Newton_Raphson(double Point_1, double Point_2, double eps_1, double eps_2, int M)
        {
            int k = 0;

            double x, y, norma, max_norma = 0, f_max_x, f_max_y, modul, det, revers_det, count = 0, t0 = 1, value_t, value;
            ...
            while (k < M)
            {
                ...
                // Шаг 10
                Console.WriteLine("ф(t0) = f(x^(" + k + ") + t0 * d^(" + k + ")) = f(" + (d[0] * t0) + "; " + (d[1] * t0) + ")");

                value_t = Sqr_x2(d[0], d[1]);
                value = Value_x(Point_1, Point_2, d[0], d[1]);
                t0 = (float)value / (float)(2 * value_t); // Находим величину шага
                Console.WriteLine("t(0) = " + t0);

                // Шаг 11
                x = Max_X(d[0], Point_1, t0);
                y = Max_Y(d[1], Point_2, t0);
                Console.WriteLine("X^(" + (k + 1) + ") = " + " (" + x + "; " + y + ")");
